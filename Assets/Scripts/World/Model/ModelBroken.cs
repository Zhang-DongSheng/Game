...
namespace Game.World
{
    ...
    /// 表现层:一比一跟随物理层子节点表现
    /// </summary>
    public class ModelBroken : MonoBehaviour
    {
        [FieldName("破碎表现系数")]
        [SerializeField] private float ratio = 300f;
        [FieldName("沉入地面时间")]
        ...
        private void Fly(Vector3 position, Vector3 direction)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                var scatter = new Vector2(Random.Range(-1f, 1f), Random.Range(-1f, 1f));
                children[i].isKinematic = false;
                children[i].AddForceAtPosition(Scatter(direction, scatter) * ratio, position);
                children[i].AddExplosionForce(ratio, children[i].transform.position, 1);
            }
        }
        ...
        private Vector3 Scatter(Vector3 direction, Vector2 scatter)
        {
            var delta = new Vector2
            {
                x = scatter.x * Mathf.Cos(scatter.y * Mathf.Deg2Rad),
                y = scatter.x * Mathf.Sin(scatter.y * Mathf.Deg2Rad),
            };
